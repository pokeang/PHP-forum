...
class user extends Db_action{
  ...
  public $user_image;
    ...
    if(@$_GET["delete"] == 1){
     ...
     $this->path_folder(FRONTVIEW.DS.VIEWFOLDER.DS.$this->obj_name.DS.$this->view_file);
	}
	function do_add(){
    $this->path_folder(FRONTVIEW.DS.VIEWFOLDER.DS.$this->obj_name.DS.$this->add_file);//view/category/add_category.php
	}
    ...
    if($this->varlidatestring(@$_GET['status']) == "status")
      ...
      $condition = "$this->prefix_field"."id <> 0";
    else if($this->varlidatestring(@$_GET['user']) == "single")
            $condition = "$this->prefix_field"."id = ".$_SESSION["id"];
    else $condition = $this->prefix_field."id=".$_SESSION["id"];
	...
	function set_info(){
		...
		$this->user_password = md5($this->varlidatestring($_POST["currentpassword"]));
    $this->newpassword = md5($this->varlidatestring($_POST["newpassword"]));    
		$this->user_rule = $_POST["userrule"];    
  ...
  function set_update(){
      ...
      $this->set_info();
      $this->userhidden = $_POST["currentspassword"];
      if((md5($this->newpassword) != $this->userhidden) && $this->newpassword != "")  {
        $this->user_password = $this->newpassword;
      }else $this->user_password = $this->userhidden;     
    }
   else  $this->id_update  = self::$field_id;
  }
  function insert_record(){
    $this->insert_into($this->table, array($this->prefix_field."username"=>$this->user_username, $this->prefix_field."email"=>$this->user_email, $this->prefix_field."fname"=>$this->user_fname, $this->prefix_field."lname"=>$this->user_lname, $this->prefix_field."password"=>$this->user_password, $this->prefix_field."role"=>$this->user_rule, $this->prefix_field."status"=>$this->user_status));
   ...
   if(isset($_POST["apply"])){
      if($this->user_username != ""){
        $this->insert_record();
         $this->user_image = $_FILES['txtphoto']["name"];
        $this->user_image != ""?$this->user_image:"no-image.jpg";
  ...
  function do_edit(){
     if(isset($_POST["close"]) || isset ($_POST["apply"])){
      if ($_FILES["txtphoto"]["name"] != "") {
       $filename = "photo/".$_SESSION["id"]."_".$_POST["oldimage"]; 
        $img_extension = substr($_FILES['txtphoto']['type'], 0, 5);
        $img_sub_extenstion = explode('/', $_FILES['txtphoto']['type']);
        $img_sub_extenstion = strtolower($img_sub_extenstion[1]);
        if ($img_extension == 'image' && $_FILES['txtphoto']["size"] < (1024 * 1024)) {
          ...
          if(file_exists($filename)){
            unlink($filename);
            unlink("photo/thumb/".$_SESSION["id"]."_".$_POST["oldimage"]);
           
          }
           $this->moveFile($_FILES['txtphoto']['tmp_name'], $_FILES['txtphoto']['name'], $img_sub_extenstion, $_SESSION['id']);
         } 
       }
       else {$this->user_image = $_POST["oldimage"];}
       
     }
	   if(isset($_POST["close"])){
        $this->set_update();
        $this->update_table($this->table, array($this->prefix_field."username"=>$this->user_username, $this->prefix_field."email"=>$this->user_email, $this->prefix_field."fname"=>$this->user_fname, $this->prefix_field."lname"=>$this->user_lname, $this->prefix_field."password"=>$this->user_password, $this->prefix_field."role"=>$this->user_rule,"image"=>$this->user_image, $this->prefix_field."status"=>$this->user_status), $this->prefix_field."id = $this->id_update");
        Message::concatSuccess("Successful for edit");
       ...
       if(mysql_num_rows($valueedit) > 0){
        while($row = mysql_fetch_array($valueedit)){
					...
					$this->user_rule = $row[$this->prefix_field."role"];
					$this->user_status = $row[$this->prefix_field."status"];
          $this->user_image = $row["image"];
        }
       $this->path_folder(FRONTVIEW.DS.VIEWFOLDER.DS.$this->obj_name.DS.$this->update_file);
      }
	} else {
