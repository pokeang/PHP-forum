<?php
class question extends Db_action{
  ...
  private $prefix_field = "que_";
    ...
    else  $id = "q.".$this->prefix_field."id asc";
		  $data = $this->query($this->select_table("$this->table q inner join tbl_users as u on q.".$this->prefix_field."user_id=u.user_id",
              "q.".$this->prefix_field."user_id=$this->que_user_id $condition", $id, "" ));
      $i=1;
  ...
  function insert_record(){
    $this->insert_into($this->table, array($this->prefix_field."text"=>$this->que_text, $this->prefix_field."top_id"=>$this->que_top_id, $this->prefix_field."user_id"=>$this->que_user_id,  $this->prefix_field."status"=>$this->que_status));
  ...
  function do_edit(){
	   if(isset($_POST["updatebtn"])){ $this->set_update();
        $this->update_table($this->table, array($this->prefix_field."text"=>$this->que_text,$this->prefix_field."top_id"=>$this->que_top_id, $this->prefix_field."user_id"=>$this->que_user_id,  $this->prefix_field."status"=>$this->que_status), $this->prefix_field."id = $this->id_update");
        Message::concatSuccess("Successful for edit");
